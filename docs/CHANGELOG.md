...
**변경:**
- `docs/ROADMAP.md`를 현재 목표, 현재 상태, 현재 결정, 다음 5개 마일스톤 중심으로 재작성했다.
- `v0.55-alpha`는 최종 아트 교체 자체가 아니라 최종 아트 교체 대비 스크린샷 QA 하네스 완료로 정리했다.
- 최종 캐릭터/오브젝트/배경 원본 수급과 교체 작업을 `docs/ART_INTAKE.md`로 분리했다.
- `v0.56-alpha`의 다음 목표를 20~30분 블라인드 플레이테스트 슬라이스 잠금으로 잡았다.
- `docs/BLIND_PLAYTEST_CHECKLIST.md`를 추가해 첫 10초, 3분, 10분, 15분, 20분, 30분 성공 기준을 정리했다.

**방향:**
- 새 시스템 추가보다 첫 제품 출시, 카드 영향 체감, 경쟁사/직원 사건, 5명 블라인드 플레이테스트를 우선한다.
...
**추가:**
- 사무실 화면의 사람, AI 에이전트, 로봇 액터를 클릭 가능한 버튼으로 바꿨다.
- 선택한 액터의 상태, 역할, 현재 작업, 체력/충성도 미터, 추천 액션을 보여주는 `OfficeActorFocusPanel`을 추가했다.
- 작업 중인 액터는 제품 메뉴로, 케어/휴식/대기 액터는 에이전트 메뉴로 이어진다.
- 액터 선택 상태, 포커스 링, 모바일 압축 표시, reduced-motion 계약을 유지했다.

**검증:**
- `npm test -- src/game/office-scene.test.ts src/ui/layout-contract.test.ts` 통과, 2 files / 28 tests
- `npm run harness:gate` 통과, 40 files / 285 tests, 데이터 검증 통과, 프로덕션 빌드 통과
- Browser QA: `http://127.0.0.1:5201/?scenario=office-visuals`에서 액터 버튼 6개, 포커스 패널 1개, 경고 액터 기본 선택, 작업 중 액터 클릭 후 `프로젝트 보기` 전환, 제품 메뉴 이동, 가로 오버플로 없음, 콘솔 오류 0건 확인
