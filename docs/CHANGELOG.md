...
**추가:**
- 장기 캠페인용 고용 후보를 22종으로 확장했다.
- 사람 직원 6종, AI 에이전트 13종, 로봇 인력 3종으로 `kind`를 명시했다.
- 차고 PM 인턴, 야간 서버 당직, 현장 영업 담당, 조직문화 매니저를 추가했다.
- 벤치마크 분석 에이전트, 시나리오 시뮬레이터, 산업 디자인 AI를 추가했다.
- 물류 로봇 클러스터와 휴머노이드 랩 어시스턴트를 추가했다.
- 아이템을 37종으로 확장하고 복지, 제조, 하드웨어, 모빌리티 카테고리를 추가했다.
- 사무실 오브젝트 매니페스트를 21종으로 확장하고 아이템 연동 배치 훅을 늘렸다.
- 아이템 아이콘 훅을 18종으로 확장했다.

**변경:**
- 아이템 카테고리 표시명에 하드웨어, 복지, 제조, 모빌리티를 추가했다.
- 콘텐츠 테스트가 사람/AI/로봇 비율, 장기 해금 조건, 선호 아이템 연결을 검증한다.

**검증:**
- `npm test -- src/game/content.test.ts src/game/asset-manifest.test.ts` 통과, 12 tests
- `npm run validate:data` 통과
- `npm run harness:gate` 통과, 126 tests, 프로덕션 빌드 통과
