...
function fillPolygon(target, points, color) {
  ...
  const maxY = Math.min(target.height - 1, Math.ceil(Math.max(...points.map((point) => point[1]))));
  ...
  for (let y = minY; y <= maxY; y += 1) {
    ...
    for (let index = 0; index < intersections.length; index += 2) {
      fillRect(target, intersections[index], y, intersections[index + 1] - intersections[index] + 1, 1, color);
    }
  }
}
...
function line(target, x0, y0, x1, y1, color, thickness = 1) {
  ...
  while (true) {
    fillRect(target, x - Math.floor(thickness / 2), y - Math.floor(thickness / 2), thickness, thickness, color);
    if (x === x1 && y === y1) break;
    const e2 = 2 * error;
    if (e2 >= dy) {
      error += dy;
...
function buildAgentSheet() {
  ...
  const columns = 3;
  const sheet = canvas(frameWidth * columns, frameHeight * manifest.agent_sprites.length * 2);
...
function buildOfficeObjectSheet() {
  ...
  const sheet = canvas(frameWidth * columns, frameHeight * rows);
...
function buildOfficeBackground() {
  const target = canvas(1280, 720, hexToRgba("#9fb7bd"));
...
writePng(path.join(outSprites, "v045-agents.png"), buildAgentSheet());
writePng(path.join(outSprites, "v045-office-objects.png"), buildOfficeObjectSheet());
writePng(path.join(outBackgrounds, "v045-isometric-office.png"), buildOfficeBackground());

console.log("Generated v0.45 isometric pixel sheets and office backdrop.");
