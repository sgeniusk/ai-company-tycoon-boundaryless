...
describe("beta readiness QA script", () => {
  ...
  it("reports the ending codex, reward, route, and scenario readiness gates", () => {
    ...
    expect(result.checks.find((check) => check.id === "ending_route_quick_start")).toMatchObject({
      label: "엔딩 Route Quick Start",
      complete: true,
    });
    expect(result.checks.find((check) => check.id === "result_route_action")).toMatchObject({
  ...
  it("prints readiness and guide status in the no-write beta readiness check", () => {
    ...
    expect(output).toContain("Readiness: 15/15 checks (100%)");
