...
- Current feature: `v0.67-alpha-multi-ending` (PLANNED — awaiting user direction at checkpoint; decision B, alt = beta prep decision C). The ENTIRE roguelike track shipped 2026-05-30: `v0.63` system spine + `v0.64` content depth + `v0.65` difficulty + `v0.66` tag-derivation engine (decision A), all COMPLETED. Baseline 48 files / 500 tests. Master plan `reports/v0_63_plus_content_roadmap.md`; post-1.0 vision `reports/v0_62_design_direction.md`.
...
- v0.64 closed 2026-05-30 — content depth (data-only roguelike variety), 2 blocks (`48a4231` 4-axis expansion 600->9,504 combos / `aec4d1d` world-events 12->26 themed + closeout), 47 files / 472 tests. No code changes — run_modifiers.json/world_events.json/validate-data + tests only; run-modifiers.ts/simulation.ts/types.ts untouched. Block #1 was coded by Codex in a parallel worktree alongside v0.63 #4.
- v0.65 closed 2026-05-30 — difficulty/challenge axis, 2 blocks (`9b2c2ff` tier foundation + additive headwind / `3e3d7bf` reward multiplier + tier-select UI + closeout), 47 files / 481 tests. #1 added RunModifiersState.challengeTier (save-migrated) + a 2-line additive difficulty hook in getMonthlyStrategicEffects (standard tier no-op); #2 multiplied getRunInsightReward by the tier reward_multiplier + tier-select UI, no new GameState field, simulation.ts/types.ts/save untouched.
- v0.66 closed 2026-05-30 — tag-derivation engine (decision A, the '무궁무진한 조합' centerpiece), 4 blocks (`dc7b579` derive-only engine / `0f01acb` cross-run discovery+도감 / `76252c2` emergent effects / #4 balance sweep+closeout), 48 files / 500 tests. Run-modifier tag combos derive named archetypes (프런티어 차고/데이터 연금술사/...); cross-run collection in roguelite.discoveredArchetypeIds; archetype monthly bonuses via the same 2-line additive hook. simulation.ts touches were only the roguelite hydrate sanitizer (#2) + the 2-line archetype effect hook (#3); #1/#4 had empty simulation.ts diffs. The whole roguelike track (v0.63 spine -> v0.64 content -> v0.65 difficulty -> v0.66 tag-engine) is complete; next is v0.67 multi-ending (decision B) or beta prep (decision C).
...
2. The roguelike track is COMPLETE (v0.63 run modifiers + v0.64 content depth + v0.65 difficulty + v0.66 tag-derivation engine all shipped). Next per `reports/v0_63_plus_content_roadmap.md` is v0.67 multi-ending (decision B) OR beta prep (decision C, real-human playtest + final art) — PENDING a user direction checkpoint.
3. Run `npm run harness:gate` baseline (48 files / 500 tests) before changes.
