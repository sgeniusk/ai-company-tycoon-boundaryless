...
- Current feature: `v0.64-alpha-content-depth` (in progress — data-only roguelike variety expansion, #1 run-modifier 4-axis content + #2 world-events content). Prior `v0.63-alpha-roguelike-run-modifiers` COMPLETED 2026-05-30 (first post-1.0 big system, §5 hold lifted by user decision; 4 blocks foundation/world-tick/yearly-events/세계뽑기-reveal, 46 files / 466 tests). Master plan `reports/v0_63_plus_content_roadmap.md`; post-1.0 vision `reports/v0_62_design_direction.md`.
...
2. Continue the roguelike track per `reports/v0_63_plus_content_roadmap.md` — v0.64 content depth (in progress), then v0.65 difficulty/challenge axis, v0.66 tag-derivation engine (decision A), v0.67 multi-ending (decision B). Roguelike run modifiers themselves shipped in v0.63.
3. Run `npm run harness:gate` baseline (46 files / 466 tests) before changes.
