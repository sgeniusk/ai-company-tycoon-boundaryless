...
- Current feature: `v0.66-alpha-tag-derivation-engine` (PLANNED — awaiting user direction; the '무궁무진한 조합' centerpiece, decision A). Roguelike track shipped end-to-end 2026-05-30: `v0.63` system spine + `v0.64` content depth + `v0.65` difficulty, all COMPLETED. Baseline 47 files / 481 tests. Master plan `reports/v0_63_plus_content_roadmap.md`; post-1.0 vision `reports/v0_62_design_direction.md`.
...
2. Continue the roguelike track per `reports/v0_63_plus_content_roadmap.md` — next is v0.66 tag-derivation engine (decision A, the '무궁무진한 조합' centerpiece) PENDING a user direction checkpoint (alternatives: v0.67 multi-ending, or beta prep). v0.63 run modifiers + v0.64 content depth + v0.65 difficulty already shipped.
3. Run `npm run harness:gate` baseline (47 files / 481 tests) before changes.
