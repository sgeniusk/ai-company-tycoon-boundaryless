...
## 현재 상태 (Current State)
**마지막 갱신** — 2026-06-21
**다음 트랙 — feat-024 크로마키 아트 재생성 (핸드오프 완료, 새 세션 시작).** imagegen 흰배경 스프라이트의 헤일로(잔상)를 근본 해결 — 흰배경 폐기 → 마젠타/녹색 크로마키 배경 생성 + 키잉 + 16색 양자화. 딥리서치(런 w4bn7b024, 23소스/21확정) 검증 — rembg neural matte는 픽셀아트 부적합, 색키잉+양자화가 정답. 대상 — 캐릭터 15컷 + 소품 + UI atlas(헤일로 진단 diagnose_white.py로 식별, halo>3 18개). 정본 `docs/codex-handoff/chroma-key-art-regen.md`, 표준 `AGENTS.md` 아트 생성 표준. **미커밋(새 세션 EditMode+캡처 검증 필요) — 캐릭터 200→248·소품 ×1.25·FAB 골드펄스 확대(GameScreen/OfficeProps, titan breaker Unity 점유로 이 세션 미검증).**
